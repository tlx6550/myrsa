...
class Myencryption{
     ...
     */
    getRasKes(data){
        this._jCryptionKeyPair(data.e,data.n,data.maxdigits,data.aeskey)
        var keys = this
        return keys
    }
    /**
     ...
     * @returns 加密字符
     * @memberof Myencryption
     */
    heRSAEncrypt(data,keys){//RSA加密方式
        var enRasStr = this._encrypt(data,keys)
        return enRasStr
    }
    _jCryptionKeyPair(encryptionExponent, modulus, maxdigits,aeskey){
     ...
     * 
     * 
     * @param {any} string 加密字符串
     * @param {any} keyPair 加密向量
     * @memberof Myencryption
     */
    _encrypt(string,keyPair){
        var that = this
        var charSum = 0
        if(string){
            try {
        ...
        var encryptedString =  that._encryption(encrypt,keyPair)
        return encryptedString
    }
    _encryption(encryptObject,keyPair){
        var charCounter = 0
        var j, block
        var encrypted = ""
        ...
        try {
            const encryptedString = encryptChar()
            // setTimeout(encryptChar, 1)
        } catch (error) {
            var obj = JSON.parse(error.message)
            if(obj.encryptedString){
                return obj.encryptedString
            }
        }
...
export default new Myencryption()